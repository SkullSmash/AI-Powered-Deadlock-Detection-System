...
function updateGraph(nodes, links, deadlock, cycle) {
    ...
    const link = svg.selectAll(".link")
        ...
        .enter().append("line")
        .attr("class", "link")
        .style("stroke", d => (deadlock && cycle.some(edge => edge[0] === d.source.id && edge[1] === d.target.id)) ? "red" : "#aaa")
        .style("stroke-width", 3)
        .style("filter", d => (deadlock && cycle.some(edge => edge[0] === d.source.id && edge[1] === d.target.id)) ? "drop-shadow(0px 0px 5px red)" : "none"); // Glowing effect for deadlock
    ...
    node.append("title")  // Tooltip for node details
        .text(d => `ID: ${d.id}`);
